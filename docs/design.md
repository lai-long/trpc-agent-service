...
| 平台限制 | 影响 | 对策 |
|---|---|---|
| 单条文本长度上限（企微约 2048 字节） | 长回复被截断或发送失败 | Outbound 消费端按字节阈值分段顺序发送，分段共享同一 trace |
...
2. 用户在 IM 内回复「确认/拒绝」：作为普通消息进入同一 `session_key`，分布式锁保证与
   其他执行串行；Worker 加载 state 检出 `pending_approval`，由 Guardrail 判定——确认人、
   调用 ID 匹配且未超时则放行原工具调用，否则终止并告知用户。
3. 超时（默认 5 分钟，租户可配）按拒绝处理，审计记 `review_timeout`。
4. 群聊场景仅消息发起人（或租户配置的审批人）的确认有效，防止群成员误确认。
